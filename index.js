...
app.post('/submitfeedback', function (req ,res){
    ...
    req.on('end', function (){
        ...
        var Q = "INSERT INTO feedback values(\'" + feedback + '\')';
        connection.query(Q, function (err, rows){
            if(err)
                console.log(err);
            res.redirect('http://192.168.1.8:8002/contest/ranking/5');
        });
    });
});
app.get('/Practice', function(req, res){
    req.session.lastPage = req.url;
    if(isAlive[req.session.stat])
        res.render("soon.ejs",{Nick : username[req.session.stat]});
    else
        res.render('soonoff.ejs')
    ...
    req.on('end', function(){
        ...
        if(validator.isEmail(email)){
            ...
            connection.query(Q,function(err,rows){
                ...
                res.redirect('http://192.168.1.8:8002');
            });
        }
    });
});
...
app.get(/contest\/id\/(.+)$/, function (req, res){
    ...
    if(contest_id > res.locals.contests || isNaN(contest_id)){
        ...
        connection.query(Q1, function (err, rows){
            ...
            connection.query(Q, function (err, rows){
                ...
                connection.query(Q2, function (err, rows){
                    your = 'ooooooooooooooooooooooooo';
    ...
    connection.query('SELECT nick, rating from profile1 where contest > 0 ORDER BY RATING DESC limit 10', function (err, rows){
            ...
            connection.query('SELECT * from upcoming', function (err, rows){
                ...
                if(isAlive[req.session.stat]){
                    ...
                    var Q = 'SELECT * from profile1 where nick = \'' + username[req.session.stat] + '\' and email != \'NULL\'';
                    console.log(Q);
                    connection.query(Q, username[req.session.stat], function (err, rows){
                        if(err){
                            console.log(err);
                            res.end();
                            return;
                        ...
                        if(rows.length == 1)
                            ...
                            res.render('submitEmail.ejs', {Nick : username[req.session.stat]});
...
app.get(/checkNick\/(.+)$/, function (req, res){
    ...
    if(req.headers.referer != 'http://192.168.1.8:8002/register'){
        res.end('sk11 rocks!');
        return;
    }
    var nick = '';
...
app.post('/submitContest', function (req, res){
    if(!isAlive[req.session.stat])
        return;
    connection.query('SELECT * from CONTEST_5 where username = ?', username[req.session.stat], function(err, rows){
        if(err){
            res.end();
            return;
        }
